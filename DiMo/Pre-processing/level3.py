...
from numpy import *
import numpy as np
from Functions import binomial,absSecond,fiveSecond,occurrences,listToString
DataNoneMotifs = []
DataMotifs = []
motifstrs = [""]
Nonemotifstrs = [""]
...
PatternsArray2 = []
tempArray = []
resultPatterns2 = [["A", 0.000, 0.000, 0.000, 0.000, 0.00, 0.00], ["C", 0.000, 0.000, 0.000, 0.000, 0.00, 0.00],
                   ["G", 0.000, 0.000, 0.000, 0.000, 0.00, 0.00],
                   ["U", 0.000, 0.000, 0.000, 0.000, 0.00, 0.00]]
j = 0
Tempchar = ''
...
for tempp in DataMotifs:
    for p5 in PatternsArray3:
        if p5[0] in tempp:
            tempc = tempc + occurrences(tempp, listToString(p5[0]))

for p in PatternsArray3:
    for s in DataMotifs:
    ...
    PatternsArray3[i][1] = TempCounter
    TempCounter = 0
    i = i + 1
# //////////////////////////
...
for p in PatternsArray3:
    ...
    PatternsArray3[inm][2] = TempCounterNoneMotif
    TempCounterNoneMotif = 0
    inm = inm + 1
...
for p in PatternsArray3:
    p[3] = format(float((int(p[1]) / 10955) * 100), '.3f')
    p[4] = format(float((int(p[2]) / 10955) * 100), '.3f')
...
correctionlevel3 = []
for p in PatternsArray3:
    if float(p[5]) > 1.5 or float(p[5]) < 0.6 or abs(int(p[1]) - int(p[2])) > 10:
        correctionlevel3.append(p)
# ///////////////////////////////////////////////////////////////////////////////////////////////
correctionlevel3.sort(key=fiveSecond, reverse=True)
correctionlevel3.sort(key=absSecond, reverse=True)

#######################################
print("ggggg", tempc)
print(TempCounter)
print(PatternsArray3)
...
with open('PatternsLevel3TSD', 'w', newline='') as csv_file:
    for p in PatternsArray3:
        csv_writer = csv.writer(csv_file, delimiter=',')
        csv_writer.writerow(p)

with open('PatternsLevel3TSD-sorted', 'w', newline='') as csv_file:
    for p in correctionlevel3:
        csv_writer = csv.writer(csv_file, delimiter=',')
        csv_writer.writerow(p)

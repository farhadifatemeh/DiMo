...
for tempp in DataMotifs:
    for p5 in PatternsArray5:
        if p5[0] in tempp:
            tempc = tempc + occurrences(tempp, listToString(p5[0]))

for p in PatternsArray5:
    for s in DataMotifs:
    ...
    PatternsArray5[i][1] = TempCounter
    TempCounter = 0
    i = i + 1
# //////////////////////////
...
for p in PatternsArray5:
    ...
    PatternsArray5[inm][2] = TempCounterNoneMotif
    TempCounterNoneMotif = 0
    inm = inm + 1
...
for p in PatternsArray5:
    p[3] = format(float((int(p[1]) / 7000) * 100), '.3f')
    p[4] = format(float((int(p[2]) / 7000) * 100), '.3f')
...
for p in PatternsArray5:
    if float(p[5]) > 1.5 or float(p[5]) < 0.5 or abs(int(p[1])-int(p[2]))>10:
        correctionlevel5.append(p)
#######################################################################
correctionlevel5.sort(key=fiveSecond,reverse=True)
correctionlevel5.sort(key=absSecond,reverse=True)
...
# // Save output
with open('PatternsLevel5TSD', 'w') as csv_file:
    for p in PatternsArray5:
        csv_writer = csv.writer(csv_file, delimiter=',')
        csv_writer.writerow(p)

with open('PatternsLevel5TSD-sorted', 'w') as csv_file:
    for p in correctionlevel5:
        csv_writer = csv.writer(csv_file, delimiter=',')
        csv_writer.writerow(p)
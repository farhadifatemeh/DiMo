...
# resultPatterns2 = []
PatternsArray2 = []
PatternsArray2=resultPatterns3
# ////////////////////////////////////////////////////
...
for tempp in DataMotifs:
    for p5 in PatternsArray2:
        if p5[0] in tempp:
            tempc = tempc + occurrences(tempp, listToString(p5[0]))

for p in PatternsArray2:
    for s in DataMotifs:
    ...
    PatternsArray2[i][1] = TempCounter
    TempCounter = 0
    i = i + 1
# //////////////////////////
...
for p in PatternsArray2:
    ...
    PatternsArray2[inm][2] = TempCounterNoneMotif
    TempCounterNoneMotif = 0
    inm = inm + 1
...
for p in PatternsArray2:
    p[3] = format(float((int(p[1]) / 13200) * 100), '.3f')
    p[4] = format(float((int(p[2]) / 13200) * 100), '.3f')
...
with open('PatternsLevel2TSD', 'w', newline='') as csv_file:
    for p in PatternsArray2:
        csv_writer = csv.writer(csv_file, delimiter=',')
        csv_writer.writerow(p)

with open('PatternsLevel2Tsd-sorted', 'w', newline='') as csv_file:

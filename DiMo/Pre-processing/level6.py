...
for tempp in DataMotifs:
    for p5 in PatternsArray6:
        if p5[0] in tempp:
            tempc = tempc + occurrences(tempp, listToString(p5[0]))

for p in PatternsArray6:
    for s in DataMotifs:
    ...
    PatternsArray6[i][1] = TempCounter
    TempCounter = 0
    i = i + 1
# //////////////////////////
...
for p in PatternsArray6:
    ...
    PatternsArray6[inm][2] = TempCounterNoneMotif
    TempCounterNoneMotif = 0
    inm = inm + 1
...
for p in PatternsArray6:
    p[3] = format(float((int(p[1]) / 5000) * 100), '.3f')
    p[4] = format(float((int(p[2]) / 5000) * 100), '.3f')
...
for p in PatternsArray6:
    if float(p[5]) > 1.5 or float(p[5]) < 0.5 or abs(int(p[1])-int(p[2]))>10:
        correctionlevel6.append(p)
#######################################################################
correctionlevel6.sort(key=fiveSecond,reverse=True)
correctionlevel6.sort(key=absSecond,reverse=True)
...
with open('PatternsLevel6TSD', 'w') as csv_file:
    for p in PatternsArray6:
        csv_writer = csv.writer(csv_file, delimiter=',')
        csv_writer.writerow(p)

with open('PatternsLevel6TSD-sorted', 'w') as csv_file:
    for p in correctionlevel6:
        csv_writer = csv.writer(csv_file, delimiter=',')
        csv_writer.writerow(p)